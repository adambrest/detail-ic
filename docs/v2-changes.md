...
A layout's parts sum to the **rounds fired** (`inputMax`), not to the credited maximum. No conduct currently separates the two — an earlier draft had the Combat Shoot LMG firing 30 rounds against a credit of 20 or 15, which was wrong: it is issued the rifles' allocation — so `inputMax` is left unset and `inputMax ?? max` reads the stage maximum everywhere.

The distinction is kept rather than deleted because it is the safer default: `parseBreakdown` and `validateStore` both validate against `inputMax ?? max`, and they disagreed at one point, which would have let a layout save but fail at scoring. Where a conduct ever does issue more rounds than it credits, the cap is applied in `scoreRows` via `Math.min(value, component.max)` while `rawHits` retains the true entry, and the score box accepts the full allocation, shows the real figure and carries an **i** note saying how many can be credited. That path is correct but currently unexercised by any shipped profile.

Entry boxes are numeric text rather than `type="number"`: the spinner arrows sit under the thumb on a phone and were easy to nudge. Out-of-range and non-numeric entries are marked as they are typed instead. Three places had to agree for this to work: the score box's own maximum, which previously came from the participant's profile rather than the rifle they fired that stage on; the detail total the UI fills in as hits are typed, which summed raw hits and so refused its own figure as out of range; and the layout dialog in Settings, which validated against the credited maximum.
...
In Combat Shoot the LMG is issued the rifles' allocation, so it defines no override and inherits their sequence: a single practice of 20 in CS (M) Stages A and C, and 5 + 10 in CS (SP).

Every stage of every conduct is now stated, which retired the *Sub-stages pending* state. That state existed because the LMG had no sequence and so no layout a user could sensibly set; it showed a closed, read-only row and let the stage be scored on its total. A missing layout can now only mean one was cleared in Settings, so every weapon gets the same **Set sub-stages** button instead. Leaving the LMG on the old path would have trapped it: a closed row, no button, and no way forward.

The guard that decides whether a layout ships at all is unchanged: a layout ships only when its sections add up to the rounds that weapon actually fires in that stage. It is what kept the LMG closed while its sequence was unknown, and it is why the override lives in the data while the arithmetic check stays in code — a rifle's default can never reach a weapon issued a different allocation, whether or not anyone remembers to exclude it. Earlier drafts generated `Part 1..4` layouts by dividing each stage total by four; that invented standards that were never confirmed, and is now blocked by a test that checks every shipped layout against the stated sequence.
